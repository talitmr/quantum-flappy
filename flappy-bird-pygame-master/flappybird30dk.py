...
class Coin(pygame.sprite.Sprite):
    ...
    def __init__(self, coin_image, coin_position):
        self.x = coin_position[0]
        self.y = coin_position[1]
        self.id= randint(0,2)
    ...
    def visible(self):
        ...
        return -Coin.WIDTH < self.x < WIN_WIDTH

    @property
    def rect(self):
        """Get the bird's position, width, and height, as a pygame.Rect."""
        return Rect(self.x + 15,  self.y+50 , Coin.WIDTH, Coin.HEIGHT)
...
class PipePair(pygame.sprite.Sprite):
    ...
    def __init__(self, pipe_end_img, pipe_body_img):
        ...
        total_pipe_body_pieces = int(
            (WIN_HEIGHT -                  # fill window from top to bottom
             ...
             4 * PipePair.PIECE_HEIGHT) /  # 2 end pieces + 1 body piece
    ...
    def get_coin_location(self):
        start_point = self.top_height_px
        end_point = WIN_HEIGHT - self.bottom_height_px
        y = randint(start_point, start_point)
...
def load_images():
    ...
    def load_image(img_file_name):
        ...
        result = pygame.image.get_extended()
    ...
    return {'background': load_image('background.png'),
            ...
            'coin': load_image('Xgate_new.png'),
            'ghost_bird' : load_image("ghost_bird.png"),
            "h_gate" : load_image("h_gate.png"),
            "measure" : load_image("measure.png")
            }
...
def main():
    ...
    images = load_images()
    states = [images["coin"],images["h_gate"],images["measure"]]
    ...
    while not done:
        ...
        if not (paused or frame_clock % msec_to_frames(PipePair.ADD_INTERVAL)):
            ...
            coin_loc = pp.get_coin_location()
            cc = Coin(choice(states), coin_loc)
            pipes.append(pp)
            coins.append(cc)
        ...
        for x in range(len(coins)):
            if coins[x].collides_with(bird):
                coin_collision = True
                index = x

        if coin_collision:
            if bird.state== 0 and coins[x].id == 0:
                bird.state = 1
            elif bird.state==0 and coins[x].id == 1:
                bird.state = 2
            elif bird.state==0 and coins[x].id == 2:
                bird.state = 0
            ###############################
            elif bird.state == 1 and coins[x].id == 0:
                bird.state = 0
            elif bird.state == 1 and coins[x].id == 1:
                bird.state = 2
            elif bird.state == 1 and coins[x].id == 2:
                bird.state = 3
            #########################################
            elif bird.state == 2 and coins[x].id == 0:
                bird.state = 2
            elif bird.state == 2 and coins[x].id == 1:
                bird.state = 0
            elif bird.state == 2 and coins[x].id == 2:
                temp = randint(0,10)
                if temp % 2 == 0:
                    bird.state = 0
                else:
        ...
        while pipes and not pipes[0].visible:
            pipes.popleft()
        while coins and coin_collision:
            coins.popleft()
        for p in pipes:
            p.update()
        ...
        for c in coins:
            c.update()
            display_surface.blit(c.image, c.rect)
